...
import { setTextField, setDateField, clearSingleSelectField, closeIssue } from "../api/graphql/mutations.mjs";
...
export default async function releaseFlow({ github, core, filter, source, releaseVersion, releaseDate, dryRun = false }) {
    ...
    if (isNaN(parsedDate.getTime())) {
        core.setFailed(`releaseDate "${releaseDate}" is not a valid date`);
        return;
    }
    const normalizedDate = parsedDate.toISOString().split("T")[0]; // YYYY-MM-DD
    ...
    async function stepSetReleaseDate(item) {
        ...
        await mutate(`setDateField release_date → "${normalizedDate}"`, () =>
            setDateField(github, { projectId, itemId: item.id, fieldId: fields.release_date.id, date: normalizedDate })
        );
        return `[done] release_date → "${normalizedDate}"`;
    }
